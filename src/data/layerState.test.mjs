...
function paramsForLayer(id) {
  ...
  if (id === 'sdpd-reports') return { windowDays: 7 };
  if (id === 'radio') {
    return {
      filter: 'all',
      volume: 0.8,
...
test('production registry is exact, canonical, and rejects incomplete contracts', async () => {
  ...
  assert.ok(REGISTERED_LAYER_IDS.includes('solar-system'));
  assert.ok(REGISTERED_LAYER_IDS.includes('home-command'));
...
test('SDPD report age window survives encode and decode', () => {
  const state = createDefaultLayerState();
  state.enabledLayerIds = ['sdpd-reports'];
  state.options['sdpd-reports'] = { windowDays: 30 };
  const params = encodeLayerStateParams(new URLSearchParams('v=2'), state);
  assert.match(String(params.get('lo') || ''), /k\.w\.30/);
  const decoded = decodeLayerStateParams(params);
  assert.equal(decoded.options['sdpd-reports'].windowDays, 30);
});
