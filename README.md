...
| ☁ **Weather**               | Current Open-Meteo conditions sampled across the visible Earth view — temperature-colored points with labels; click a sample for wind, cloud, and the WMO condition. Density follows camera altitude. Not radar and not a forecast loop                                                                                                                                              | Open-Meteo                              | 🟢                                                                                                  |
| 🚨 **SDPD Reports**         | San Diego Police Department NIBRS offenses at hundred-block locations — choose 1, 7, 30, or 90 days on the layer row, then click a point for offense, neighborhood, and case number. Informational only; not for operational use                                                                                                                                                      | City of San Diego Open Data             | 🟢                                                                                                  |
| ▣ **Geofence Watch**        | Watch zones from gitignored `config/geofences.json` — first zone is Barrio Logan south and west of I-5. Enable the layer (or WATCH → WATCHING) to log matching SDPD reports, draw the fence, and fly to a logged event from the WATCH panel                                                                                                                                         | Local checkout file + SDPD              | 🟢                                                                                                  |
| ◈ **Personal Tags**         | Your own labeled places from gitignored `config/location-tags.json` — right-click the globe to copy a paste-ready place, drop it into `places`, then open LOCATION → Saved to fly there. Enable Data Layers → Utilities → Personal Tags to see markers; click a marker to inspect it without moving the camera                                                                                                                                 | Local checkout file                     | 🟢                                                                                                  |
